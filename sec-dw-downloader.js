...
let CONFIG = {
  ...
  dateFrom: null,
  dateTo: null,
  filterIdentifiers: null, // Set<string> of identifiers to filter by, or null for no filter
};
...
function displayBanner() {
  ...
  console.log(
    chalk.gray("  Thailand Derivative Warrant Terms & Conditions Downloader\n"),
  );
}
...
async function getUserInputs() {
  ...
  // Ask about global identifier filtering
  const { useFilter } = await inquirer.prompt([
    {
      type: "confirm",
      name: "useFilter",
      message: "Filter files by global identifier?",
      default: false,
    },
  ]);
    ...
    const filterAnswers = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (
            ![".xlsx", ".xls"].includes(path.extname(trimmed).toLowerCase())
          ) {
      ...
      },
      {
        ...
        validate: (input) =>
          input.trim() ? true : "Please enter a column name",
        transformer: (input) => chalk.cyan(input),
      },
    ]);

    // Read identifiers from Excel file
    try {
      ...
      CONFIG.filterIdentifiers = identifiers;
      console.log(
        chalk.green(`\n✓ Loaded ${identifiers.size} identifiers for filtering`),
      );
    } catch (err) {
      console.log(
...
function displayConfig() {
  ...
  console.log(
    `  ${chalk.white("Date Range:")} ${CONFIG.dateFrom} to ${CONFIG.dateTo}`,
  );
  console.log(
    `  ${chalk.white("Download Folder:")} ${path.resolve(CONFIG.downloadDir)}`,
  );
  console.log(
    `  ${chalk.white("Parallel Downloads:")} ${CONFIG.concurrentDownloads}`,
  );
  if (CONFIG.filterIdentifiers) {
    console.log(
      `  ${chalk.white("ID Filter:")} ${chalk.yellow("ON")} (${CONFIG.filterIdentifiers.size} identifiers)`,
    );
  } else {
    console.log(
      `  ${chalk.white("ID Filter:")} ${chalk.gray("OFF (downloading all)")}`,
    );
  }
  console.log(chalk.gray("─".repeat(50)));
}
...
function displaySummary(results) {
  ...
  if (successful.length > 0) {
    ...
    console.log(
      `  ${chalk.white("📦 Total size:")} ${(totalSize / 1024 / 1024).toFixed(2)} MB`,
    );
  }

  console.log(
    `  ${chalk.white("📁 Location:")} ${path.resolve(CONFIG.downloadDir)}`,
  );
  console.log(chalk.gray("─".repeat(50)));
  ...
  const { tool } = await inquirer.prompt([
    {
      ...
      choices: [
        {
          name: "📄 Documents Downloader  — Download DW terms & conditions from SEC",
          value: "downloader",
        },
        {
          name: "🔎 ISIN Finder           — Look up ISIN codes",
          value: "isin",
        },
        {
...
async function main() {
  try {
    ...
    if (stats.completed > 0) {
      console.log(
        chalk.gray(
          `  (${stats.completed} already downloaded, ${pendingTransIDs.length} remaining)\n`,
        ),
      );
    }
